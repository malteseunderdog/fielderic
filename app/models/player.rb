...
class Player < ActiveRecord::Base
  ...
  attr_accessible :nickname, :email, :name, :password, :password_confirmation
  ...
  def self.authenticate(nickname, password)
    ...
    u = find_by_login(nickname.downcase) # need to get the salt
  ...
  def self.find_by_fb_user(fb_user)
    Player.find_by_fb_user_id(fb_user.uid) || Player.find_by_email_hash(fb_user.email_hashes)
  end
  ...
  def self.create_from_fb_connect(fb_user)
    new_facebooker = Player.new(:name => fb_user.name, :nickname => "#{fb_user.uid}", :password => "", :email => "")
  ...
  def link_fb_connect(fb_user_id)
    unless fb_user_id.nil?
      ...
      existing_fb_user = Player.find_by_fb_user_id(fb_user_id)
